...
1. After data manipulation stage (which is fully described in "Cliona Prevalence- Coral Health Data" section) the first step is to create a binary presence column for Cliona. which creates a new column, indicating whether a Cliona sponge was detected for every single coral observation beginning in 2005-2024, this accumulates to 56,605 total observations. Logic: Extracts only species (SPP) and Cliona presence (presence).
...
7. Next step is to define the smallest Cliona prevalence worth detecting. The variable p1 is set to 0.004, which represents the smallest Cliona prevalence that is considered biologically meaningful for detection.
```{r,warning = FALSE, message = FALSE}
# Define expected prevalence rates
p1 <- 0.004  # Smallest Cliona prevalence worth detecting
```
